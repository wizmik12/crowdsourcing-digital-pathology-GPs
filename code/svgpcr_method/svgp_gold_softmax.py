...
if not os.path.exists('./Results/'):
    os.makedirs('./Results/')
#Loading data
X_train, y_train = np.load("./features_pretrained/X_train_512pool.npy").astype('float64'), np.load("./features_pretrained/y_train.npy").astype('float64')
...
m = gpflow.models.SVGP(X=X_train_sub, Y=y_train_sub, Z=Z, kern=gpflow.kernels.RBF(X_train_sub.shape[1], variance=2.0, lengthscales =2.0, ARD=False), likelihood=gpflow.likelihoods.SoftMax(3), num_latent=3, minibatch_size=minibatch_size)
...
with open('./Results/results_svgp_gold_softmax.txt', 'w') as the_file:
    the_file.write(metrics)
    the_file.write('\n\nAUC  ' + str(roc_macro) +'\nNLL ' + str(NLL))


#saver = gpflow.saver.Saver()
#saver.save("svgp_gold.gpflow", m)
